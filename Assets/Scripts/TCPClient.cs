...
#else
        // connect to server
        using (System.Net.Sockets.TcpClient client = new System.Net.Sockets.TcpClient(host, Int32.Parse(port)))
...
#if UNITY_EDITOR
    private async void SendFileUWP(string filename = "")
#else
    public async Task SendFileUWP(string filename = "")
#endif
    {
#if UNITY_EDITOR
        errorStatus = "Unity TCP client used in UWP!";
#else
        // connect to server
        using (Windows.Networking.Sockets.StreamSocket socket = new Windows.Networking.Sockets.StreamSocket()) 
        {
            Windows.Networking.HostName serverHost = new Windows.Networking.HostName(host);
            await socket.ConnectAsync(serverHost, port);

            using (Stream streamOut = socket.OutputStream.AsStreamForWrite())
            using (StreamWriter writer = new StreamWriter(streamOut) { AutoFlush = true })
...
#else 
        await SendFileUWP(filename: filename);
#endif
